...
-- AUTH-3 forward fix: PostgreSQL exposes whitespace trimming as btrim(text).
-- Keep the deployed function contract intact.
...
begin
  ...
  if p_prepared_habit is not null then
    if pg_catalog.jsonb_typeof(p_prepared_habit) <> 'object'
       or pg_catalog.nullif(pg_catalog.btrim(p_prepared_habit->>'name'), '') is null
       or pg_catalog.nullif(pg_catalog.btrim(p_prepared_habit->>'habit_type'), '') is null
    ...
    v_habit := pg_catalog.jsonb_build_object(
      ...
      'family_id', nullif(pg_catalog.btrim(p_prepared_habit->>'family_id'), ''),
      'emoji', nullif(pg_catalog.btrim(p_prepared_habit->>'emoji'), ''),
      ...
      'unit', nullif(pg_catalog.btrim(p_prepared_habit->>'unit'), ''),
  ...
  v_name := nullif(pg_catalog.btrim(p_profile->>'name'), '');
