...
-- AUTH-3 forward fix: keep the legacy completion flag consistent with the
-- transactional onboarding status and make completed replays self-healing.
...
begin
  ...
  if v_existing.status = 'completed' then
    ...
    -- A completed ledger row is authoritative for this exact operation. It
    -- may repair only the legacy completion flag; no profile payload, habit,
    -- ledger row, reward, or operation identity is recreated.
  ...
  if v_effective_resolution = 'newAccount' then
    insert into public.profiles (
      ...
      v_user_id, v_name, 'completed', 1, true, pg_catalog.now()
    )
    ...
    if v_inserted_profile_count = 0 then
      ...
      set onboarding_status = 'completed',
          ...
          onboarding_version = greatest(onboarding_version, 1)
      where id = v_user_id;
    end if;
  else
    update public.profiles
