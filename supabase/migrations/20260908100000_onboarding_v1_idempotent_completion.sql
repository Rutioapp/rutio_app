...
-- AUTH-3 keeps the operation ledger private.  The client can call the RPC,
-- but cannot read, insert, update, or delete ledger rows directly.
...
create table if not exists app_private.onboarding_completion_operations (
  ...
  constraint onboarding_completion_operations_status_check
    check (status in ('in_progress', 'completed')),
  constraint onboarding_completion_operations_fingerprint_check
    check (length(payload_fingerprint) = 32)
);

create unique index if not exists
  onboarding_completion_operations_user_operation_uidx
  on app_private.onboarding_completion_operations (user_id, operation_id);

comment on table app_private.onboarding_completion_operations is
  'Private AUTH-3 idempotency ledger. Payload contains only the canonical completion contract; never credentials or tokens.';
comment on column app_private.onboarding_completion_operations.payload is
  'Canonical JSONB fingerprint input: accountResolution, preparedHabitDecision, approved name, canonical prepared habit, and reminder intent.';
...
declare
  ...
  v_decision text := pg_catalog.lower(pg_catalog.trim(coalesce(p_prepared_habit_decision, '')));
  v_client_resolution text := pg_catalog.lower(pg_catalog.trim(coalesce(p_account_resolution, '')));
  v_name text;
  v_habit jsonb;
...
begin
  ...
  if p_prepared_habit is not null then
    if pg_catalog.jsonb_typeof(p_prepared_habit) <> 'object'
       or pg_catalog.nullif(pg_catalog.trim(p_prepared_habit->>'name'), '') is null
       or pg_catalog.nullif(pg_catalog.trim(p_prepared_habit->>'habit_type'), '') is null
       or not public.is_valid_habit_schedule(p_prepared_habit->'schedule') then
      raise exception using errcode = '22023', message = 'invalid_payload';
    end if;
    if pg_catalog.lower(pg_catalog.trim(p_prepared_habit->>'habit_type')) not in ('check', 'count') then
      raise exception using errcode = '22023', message = 'invalid_payload';
    end if;
    if pg_catalog.lower(pg_catalog.trim(p_prepared_habit->>'habit_type')) = 'count'
    ...
    v_habit := pg_catalog.jsonb_build_object(
      'name', pg_catalog.trim(p_prepared_habit->>'name'),
      'family_id', nullif(pg_catalog.trim(p_prepared_habit->>'family_id'), ''),
      'emoji', nullif(pg_catalog.trim(p_prepared_habit->>'emoji'), ''),
      'habit_type', pg_catalog.lower(pg_catalog.trim(p_prepared_habit->>'habit_type')),
      'target_count', case when (p_prepared_habit->>'target_count') is null then null else (p_prepared_habit->>'target_count')::integer end,
      'unit', nullif(pg_catalog.trim(p_prepared_habit->>'unit'), ''),
  ...
  v_name := nullif(pg_catalog.trim(p_profile->>'name'), '');
