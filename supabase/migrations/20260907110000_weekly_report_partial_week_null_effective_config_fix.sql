...
-- Forward fix for NULL composite results from the scalar effective-config helper.
-- Keep the helper contract unchanged; every generator caller validates the
-- composite before dereferencing or inserting its fields.
...
begin
  ...
  loop
    ...
    loop
      ...
      if not found
         or v_cfg.id is null
         or v_cfg.effective_local_date is null
         or v_cfg.schedule is null
         or v_cfg.effective_local_date > v_day
         or v_cfg.is_archived
         or v_cfg.schedule->>'type' is distinct from 'timesPerWeek' then
        continue;
      end if;
